...
for t = 1:length(time) - 1
    plot(5,0,'rO');
    hold on;
    lineX = fish_X(t) - round(fish_X(t));
    plot([0,10],[lineX,lineX]);
    plot([0,10],[lineX+1,lineX+1]);
    plot([0,10],[lineX+2,lineX+2]);
    plot([0,10],[lineX-1,lineX-1]);
    plot([0,10],[lineX-2,lineX-2]);
    if(fish_X(t) <= 2.5 || fish_X(t) >= -2.5)
        plot([0,10],[fish_X(t),fish_X(t)],'r');
    end
    text(0,0,'\downarrow','fontsize',3)
    hold off;
    xlim([0,10]);
    ylim([-2.5,2.5])
%     plot(time(1:t),fish_X(1:t),'g');
%     hold on
%     plot(time(t),fish_X(t),'O');
%     hold off
    pause(0.01);
