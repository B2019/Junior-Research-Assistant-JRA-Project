...
replayT = 600; %Amount of replay time
phases = 6; %Number of different gain phases in recorded (replay) time
...
for t = 1:(T - replayT)/dt - 1
    ...
    if(time(t) >500 & time(t) <550)
        I=10;
    else
        I=0;
    end
    M(t+1) = M(t) + 1/tau_b * dt * (-M(t) + w_M * M(t) + w_MS * S(t)); %Update motor node
    S(t+1) = S(t) + 1/tau_b * dt * (-S(t) + w_S * S(t) + w_SM * M(t) - (swim_V(t) + flow_V) + randn / sqrt(dt)+I); %Update sensor node
    X1(t+1) =  w_SM * M(t);
